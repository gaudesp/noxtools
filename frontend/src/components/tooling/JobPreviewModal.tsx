  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div
        ...
        tabIndex={-1}
      />
      <div className="relative z-10 w-full max-w-4xl bg-slate-950 border border-slate-800 rounded-xl shadow-2xl p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-white">Job {job.id}</h2>
            <p className="text-xs text-slate-400 mt-1">{job.input_filename || "Unknown file"}</p>
            <div className="flex items-center gap-2 mt-2">
              <JobStatusBadge status={job.status} />
              <span className="text-[11px] uppercase tracking-wide text-slate-400">{job.tool}</span>
            </div>
          </div>
          <button
            type="button"
