"""Noxtunizer service for uploads, file handling, and Essentia analysis."""
...
from app.executors.noxtunizer_executor import NoxtunizerExecutor
from app.models.job import Job, JobTool, JobUpdate
from app.services.job_service import JobService


class NoxtunizerService:
  """Handles Noxtunizer uploads and processing."""

  BASE_UPLOAD = Path("media/noxtunizer/uploads")
  BASE_OUTPUT = Path("media/noxtunizer/outputs")

  def __init__(self, job_service: JobService) -> None:
    self.job_service = job_service
    self.BASE_UPLOAD.mkdir(parents=True, exist_ok=True)
    self.BASE_OUTPUT.mkdir(parents=True, exist_ok=True)
    self.executor = NoxtunizerExecutor(base_output=self.BASE_OUTPUT)
